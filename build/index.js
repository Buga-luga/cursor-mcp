...
import { findClineExtension, findCursorExecutable, openClineNewTab } from "./handlers/OpenCursorHandler.js";
...
const serverTools = {
    ...
    "task_create": {
        ...
        handler: async (params) => {
            const taskId = await taskHandler.createTask(params.title, params.description, params.files, params.assignTo);
            return {
                _meta: {},
                content: [{
                        type: "text",
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
        const tool = serverTools[request.params.name];
        if (!tool) {
