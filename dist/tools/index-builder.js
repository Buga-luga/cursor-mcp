...
export const indexBuilderTool = {
    ...
    handler: async (params) => {
        try {
            const { rootDir, include = ['**/*.{js,jsx,ts,tsx,py,java,cpp,c,h,hpp}'], exclude = ['**/node_modules/**', '**/dist/**', '**/.git/**'], maxFileSizeBytes = 1024 * 1024, // 1MB default max file size
            chunkSize = 1000, overlapSize = 100 } = params;
            // Get all matching files
            const files = await Promise.all(include.map(pattern => globAsync(pattern, {
                cwd: rootDir,
                ignore: exclude,
                absolute: true,
                nodir: true
