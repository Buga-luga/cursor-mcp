...
type EmbeddingParams = z.infer<typeof EmbeddingParamsSchema>
...
async function getEmbeddings(text: string, model: string = 'text-embedding-ada-002'): Promise<number[]> {
...
export const computeEmbeddingsTool: MCPTool<EmbeddingParams> = {
  ...
  handler: async (params: EmbeddingParams) => {
    try {
      const {
        content,
        chunkSize = 1000,
