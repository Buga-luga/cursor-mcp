...
type WatcherParams = z.infer<typeof WatcherParamsSchema>
...
function getCurrentIndexingProject(cursorConfigPath: string, basePath: string): string | null {
  try {
    // Read Cursor's current workspace state
    const workspaceStatePath = join(cursorConfigPath, 'workspace-state.json')
    if (!existsSync(workspaceStatePath)) return null
    ...
    if (relativePath.startsWith('..')) return null // Path is outside basePath
...
export const cursorIndexWatcherTool: MCPTool<WatcherParams> = {
  ...
  handler: async (params: WatcherParams) => {
    try {
      ...
      console.log('Watching for Cursor indexing in projects:', 
        projectPaths.length > 0 ? projectPaths : ['all under ' + basePath])
...
async function getCursorIgnorePatterns(projectDir: string): Promise<string[]> {
  try {
    const cursorIgnorePath = join(projectDir, '.cursorignore')
    const content = await import('fs').then(fs => 
      fs.readFileSync(cursorIgnorePath, 'utf-8')
    )
    return content
      .split('\n')
      .map(line => line.trim())
