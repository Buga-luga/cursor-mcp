...
type SendToClaudeParams = z.infer<typeof SendToClaudeParamsSchema>

export const sendToClaudeTool: MCPTool<SendToClaudeParams> = {
  name: 'send_to_claude',
  description: 'Format and send the codebase index to Claude in a way it can understand',
  inputSchema: SendToClaudeParamsSchema,
  handler: async (params: SendToClaudeParams) => {
    try {
...
function formatStructure(obj: Record<string, any>, prefix = ''): string {
  let result = ''
