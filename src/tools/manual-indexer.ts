...
type ManualIndexParams = z.infer<typeof ManualIndexParamsSchema>

export const manualIndexerTool: MCPTool<ManualIndexParams> = {
  name: 'index_path',
  description: 'Manually index any directory path and optionally send it to Claude',
  inputSchema: ManualIndexParamsSchema,
  handler: async (params: ManualIndexParams) => {
    try {
      const { 
        path,
      ...
      const formattedResult = await sendToClaudeTool.handler({
        index: indexResult.content[0].text as any,
